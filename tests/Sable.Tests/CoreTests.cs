...
public class SettingsTests
{
    ...
    [Fact]
    public void SmartSelectGrid_ExplicitLevels_AndAutoScalesByVram()
    {
        const ulong gb = 1024UL * 1024 * 1024;
        // explicit levels ignore VRAM
        Assert.Equal(12, SableSettings.SmartSelectGrid(SmartSelectQuality.Fast, 999 * gb));
        Assert.Equal(16, SableSettings.SmartSelectGrid(SmartSelectQuality.Balanced, 0));
        Assert.Equal(32, SableSettings.SmartSelectGrid(SmartSelectQuality.Thorough, 0));
        // auto: 0 (unknown) and low VRAM → conservative; scales up with VRAM
        Assert.Equal(12, SableSettings.SmartSelectGrid(SmartSelectQuality.Auto, 0));
        Assert.Equal(12, SableSettings.SmartSelectGrid(SmartSelectQuality.Auto, 4 * gb));   // 4 GB laptop
        Assert.Equal(16, SableSettings.SmartSelectGrid(SmartSelectQuality.Auto, 8 * gb));
        Assert.Equal(32, SableSettings.SmartSelectGrid(SmartSelectQuality.Auto, 16 * gb));
    }
