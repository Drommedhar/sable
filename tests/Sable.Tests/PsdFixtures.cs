...
public static class PsdFixtures
{
    ...
    private sealed class Builder
    {
        ...
        public bool Psb;                 // large document format: version 2 + 8-byte section lengths

        public byte[] Build(byte[]? layerInfo, byte[]? composite = null)
        {
            W32(0x38425053); W16(Psb ? (ushort)2 : (ushort)1);
            ...
            int liHdr = Psb ? 8 : 4;   // the layer-info length field is 8-byte in PSB
            if (layerInfo is null) WLen(0);
            else { WLen((ulong)(liHdr + layerInfo.Length)); WLen((ulong)layerInfo.Length); _ms.Write(layerInfo); }
            if (composite is not null) _ms.Write(composite);
            return _ms.ToArray();
        }
        void WLen(ulong v) { if (Psb) W64(v); else W32((uint)v); }
        ...
        void W64(ulong v) { Span<byte> b = stackalloc byte[8]; BinaryPrimitives.WriteUInt64BigEndian(b, v); _ms.Write(b); }
    }
    ...
    private sealed class Layers
    {
        ...
        private int _count;
        public bool Psb;   // large document format: 8-byte channel-data lengths
            ...
            bool gray = false, int comp = 0)
        {
            ...
            foreach (var (id, plane) in chans)
            {
                ...
                if (Psb) W64(_recs, (ulong)(2 + payload.Length)); else W32(_recs, (uint)(2 + payload.Length));
        ...
        static void W64(Stream s, ulong v) { Span<byte> b = stackalloc byte[8]; BinaryPrimitives.WriteUInt64BigEndian(b, v); s.Write(b); }
    ...
    private static byte[] VLong(int v) { using var ms = new MemoryStream(); ms.Write("long"u8); W32(ms, (uint)v); return ms.ToArray(); }
    ...
    /// <summary>§14: a Smart Object (SoLd) layer — rasterised, but its placement + identity are captured
    /// into <see cref="Sable.Engine.Layers.SmartObjectInfo"/> (Tier 1, plans/SMART_OBJECTS.md).</summary>
    public static byte[] SmartObjectRasterised()
    {
        using var sold = new MemoryStream();
        sold.Write("soLD"u8);   // signature
        W32(sold, 5);           // version
        W32(sold, 16);          // descriptor version
        sold.Write(Desc(
            ("Idnt", VText("uid-fixture")),
            ("Trnf", VlLs(VDoub(0), VDoub(0), VDoub(16), VDoub(0), VDoub(16), VDoub(16), VDoub(0), VDoub(16))),
            ("Sz  ", VObjc(("Wdth", VDoub(32)), ("Hght", VDoub(24)))),
            ("Type", VLong(2))));
        var li = new Layers();
        li.Add("Placed 1", 0, 0, 8, 8, fill: (50, 100, 150, 255), tagged: Tagged("SoLd", sold.ToArray()));
        return new Builder { Width = 8, Height = 8 }.Build(li.Build());
    }

    /// <summary>PSB (large document format): version 2 + 64-bit section/channel lengths, two raster
    /// layers — must import identically to the PSD equivalent (plans/PSB_FEASIBILITY.md Tier 1).</summary>
    public static byte[] PsbTwoLayers()
    {
        var li = new Layers { Psb = true };
        li.Add("Back", 0, 0, 4, 4, fill: (10, 20, 30, 255));
        li.Add("Front", 0, 0, 4, 4, fill: (200, 100, 50, 255), op: 128, blend: "mul ");
        return new Builder { Width = 4, Height = 4, Psb = true }.Build(li.Build());
    }

    /// <summary>PSB whose canvas exceeds the in-memory pixel budget → honest rejection (Tier 2 pending).</summary>
    public static byte[] PsbOversize()
        => new Builder { Width = 20000, Height = 20000, Psb = true }.Build(null);
