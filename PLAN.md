...
- ✅ **shape stroke/fill/dash + more shapes**: `ShapeLayer` now has fill on/off + stroke on/off (separate colour/width) + **dashed stroke** (dash/gap) + **RoundedRect/Polygon/Star/Arrow** (corner radius, n-sides, star inner ratio, arrowhead). Shared `VectorRaster` (even-odd fill + dash-aware distance stroke) used by shapes AND paths. **Shape options bar** (draw-time fill/stroke/dash/sides/radius defaults) + **modeless Shape properties panel** (`ShapeWindow`, edits the selected shape live). Serialized; 7 raster tests + roundtrip + GPU smoke. ✅ **stroke joins/caps** (miter/round/bevel + butt/round/square in `VectorRaster.Stroke`; Shape-panel Cap/Join combos; serialized). _(boolean ops · custom shapes → §16.16.)_
- ✅ **text depth**: **area/frame text** (`TextLayer.BoxWidth` → word-wrap) + **tracking** (letter spacing); options-bar Box W + Tracking fields. **Text→curves** (Type ▸ Convert to Curves): glyph outlines → editable `PathLayer` with multi-contour fill (`PathLayer.ExtraContours` + `VectorRaster.FillMulti` so counters/holes work, e.g. "O"). **Text-on-path** (Type ▸ Fit/Detach Text to Path): glyphs flow along the topmost vector path/shape (`TextRaster.RenderOnPath`, arc-length glyph placement + tangent rotation; baked polyline `TextLayer.PathPoints`). Serialized; 8 tests + GPU smoke. ⬜ live (non-baked) path link · character/paragraph panels (kerning/super-sub/lists) · text styles.
- ✅ **pen/bézier path** (`PathLayer`: cubic-bézier node list + closed + fill + stroke; CPU rasterise = flatten→4×-supersampled even-odd fill + distance-field stroke; compositor `GetPathBuffer`; serialized). **Pen tool** (`ToolKind.Pen`, key P): click=corner node, drag=mirrored smooth handles, click-first-node/Enter=finish (closed/open), Esc=cancel; live preview (curve spine + node/handle markers) via a new blit storage buffer (binding 6). Closed→filled, open→stroked, brush colour.
- ✅ **node-editing tool** (`ToolKind.Node`, P cycles Pen/Node): drag anchor/handle (smooth mirrors, Alt breaks), Alt-click=delete node, click-path=insert node (de Casteljau split), undoable `EditPathCommand`. ⬜ explicit corner↔smooth toggle button.
...
### Phase 4 — Vector & text depth ✅ DONE
