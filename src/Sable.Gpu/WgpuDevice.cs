...
public sealed unsafe class WgpuDevice : IDisposable
{
    ...
    /// <summary>
    /// Build the wgpu API binding. Silk.NET.Core 2.23's default Linux loader does NOT honour the
    /// deps.json <c>runtimes/&lt;rid&gt;/native</c> asset path (works on Windows, fails on Linux with
    /// "Could not load from any of the possible library names"). Resolve the shipped native library
    /// explicitly so the loader receives a full path; fall back to <see cref="WebGPU.GetApi"/> if the
    /// file isn't where we expect (e.g. a custom deployment layout).
    /// </summary>
    private static WebGPU CreateApi()
    {
        var path = ResolveNativeLibPath();
        if (path is not null)
        {
            try { return new WebGPU(new DefaultNativeContext(path)); }
            catch { /* fall through to Silk's default resolution */ }
        }
        return WebGPU.GetApi();
    }

    /// <summary>Absolute path to the bundled wgpu_native for the running OS/arch, or null if not found.</summary>
    private static string? ResolveNativeLibPath()
    {
        string file = OperatingSystem.IsWindows() ? "wgpu_native.dll"
        ...
        string[] candidates =
        {
            Path.Combine(baseDir, "runtimes", $"{os}-{arch}", "native", file),  // framework-dependent layout
            Path.Combine(baseDir, file)                                          // flattened (published with a RID)
        };
